...
def main():
    ...
    env = UnityEnvironment(file_name='../Reacher_Linux_2/Reacher_Linux/Reacher.x86_64')
    brain_name = env.brain_names[0]
    brain = env.brains[brain_name]
    env_info = env.reset(train_mode=True)[brain_name]
    num_agents = len(env_info.agents)
    ...
    agent_groups = [
        ...
        create_agent_group("ddpg2", ddpg.Agent, ddpg.ReplayBuffer, action_size, num_agents, buffer_size=int(1e5),
                           batch_size=512, modified_layers=False, weight_decay=0),
    ]
    epochs = 0
    while agent_group_below_min_score(agent_groups, 35):
        epochs += 1
        for agent_group in agent_groups:
            print(f"starting episode {agent_group['episodes']} for {agent_group['name']}")
...
def create_agent_group(name, agent_constructor_fn, memory_constructor_fn, action_size, num_agents, buffer_size=int(1e6),
    ...
    agents = [agent_constructor_fn(state_size=33, action_size=action_size, random_seed=2, memory=memory, step_slot=i,
...
def train_episode(env, agent_group):
    ...
    while True:
        ...
        # to do update learning every 20 time steps
        ...
        if np.any(dones):  # exit loop if episode finished
            break
        if (steps % 200) == 0:
            print(f"Step Average At {steps} is {np.mean(scores_episode)} for {agent_group['name']}")

    # scores is a list containing all the episode scores
    agent_group["scores"].append(np.mean(scores_episode))
    # scores deque contains the last 100 episode scores
    agent_group["scores_deque"].append(np.mean(scores_episode))
...
def write_group_performance(agent_groups):
    ...
    header = ""
    for agent_group in agent_groups:
        header += f"{agent_group['name']}_score, {agent_group['name']}_deque_score_mean,"
        num_episodes = max(num_episodes, agent_group['episodes'])

    # remove last comma and add carriage return
    header = header[:-1] + "\n"
    lines = [header]
    ...
    for episode in range(0, num_episodes):
        line = ""
        for agent_group in agent_groups:
            line += f"{agent_group['scores'][episode]},{np.mean(agent_group['scores_deque'])},"
        line = line[:-1] + "\n"
        lines.append(line)
